...
function ProjectDetail() {
  ...
  // Fetch collaboration requests for this idea
  useEffect(() => {
    api
      .get(`/collaboration/idea/${id}`)
      .then((data) => {
    ...
    return (
      <div className="page-shell">
        <section className="surface-card glass-ring p-10 text-center">
          <p className="text-sm text-slate-300">Loading idea...</p>
        </section>
      </div>
    );
  }
  ...
  const handlePostComment = async () => {
    ...
    try {
      const data = await api.post(`/ideas/${id}/comments`, {
  ...
  const handleRequestCollaboration = async (event) => {
    event.preventDefault();

    if (!isAuthenticated) {
      setRequestMessage("Please log in to send collaboration requests.");
      return;
    }

    try {
      const request = await requestCollaboration(idea.id, collaborationMessage);
      if (!request) {
      ...
      setCollabRequests((current) => [request, ...current]);
    } catch (err) {
      setRequestMessage(err.message || "Could not send request.");
    }
  };

  const isSaved = isProjectSaved(idea.id);
  const isOwnedByCurrentUser = user && String(idea.ownerId) === String(user.id);
  const verdictToneClass =
    idea.verdict === "Real Problem"
  ...
  const handleRemoveProject = async () => {
    const shouldRemove = window.confirm(
      `Delete "${idea.title}" from the community feed? This will remove its comments and collaboration requests too.`
    );

    if (!shouldRemove) return;

    try {
      await removeProject(idea.id);
      navigate("/works");
    } catch (err) {
      ...
      <section className="surface-card glass-ring overflow-hidden p-8 sm:p-10">
        <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl space-y-4">
            ...
            <h1 className="page-title text-4xl sm:text-5xl">{idea.title}</h1>
            <p className="page-copy">{idea.summary}</p>
          </div>
        ...
        <div className="mt-8 flex flex-wrap gap-3 border-t border-white/10 pt-6 text-xs uppercase tracking-[0.22em] text-slate-400">
          <span>Posted by {idea.ownerName || "Community Builder"}</span>
          <span>
            Posted{" "}
      ...
      <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
        <article className="surface-card glass-ring p-8">
          ...
          <div className="mt-4">
            <ReactMarkdown components={markdownComponents}>
              {[idea.summary, idea.analysis].filter(Boolean).join("\n\n")}
            </ReactMarkdown>
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {isAuthenticated && (
              <button
                ...
                onClick={() => toggleSavedProject(idea.id)}
        ...
        <article className="surface-card glass-ring p-8">
          <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-5">
            ...
            <span className="text-sm text-slate-400">{comments.length} posts</span>
          </div>
              ...
              comments.map((entry) => (
                <article key={entry.id} className="surface-panel px-5 py-4">
                  <div className="flex items-center gap-2">
                    <strong className="text-sm font-semibold text-sky-200">
          ...
          <div className="mt-6 grid gap-4">
            {isAuthenticated ? (
              ...
              <p className="text-sm text-slate-400">
                <Link to="/login" className="text-sky-200 underline">Log in</Link> to join the discussion.
              </p>
            )}
          </div>
        </article>
